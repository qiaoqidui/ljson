...
#define LJSON_STACK_SIZE        (1*10)  /* type (1 byte) for object '{', array '[', string '"' */
#define LJSON_BUFFER_SIZE       256     /* buffer (with '\0') for key, value */
#define LJSON_CONTEX_STACK_SIZE (10*9)  /* contex (10 bytes) for object '{', array '[' */
...
typedef struct _ljson_parser
{
    uint8_t state;
    lstack_t stack;
    uint8_t stack_buffer[LJSON_STACK_SIZE];
    uint8_t *pval;
